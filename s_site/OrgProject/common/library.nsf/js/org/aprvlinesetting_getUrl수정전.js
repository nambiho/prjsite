;(function (realm) {
	...
	setDisplay=function(){
		...
		_this.$ajax({
			url:"/"+system.userprofile_path+"/wvAprvLine?readviewentries&outputformat=json&count=-1&restricttocategory="+category,
			dataType:"json"
		...
		if (lastApprove) {
			...
			util.addEvent(lastApprove,"change",function () {
				var approveList=util.getElementById("approveList"),
				html="",i=0;
				for(;i<this.selectedIndex;i++){
					html+=util.pattern({
						value:this.options[i].value,
						disp:this.options[i].text
					},pattern.approveline);
				}
				_this.setCount("approveList",i);
				cooperPosition.apply(_this,[this.selectedIndex,title,dept,name]);
	...
	getherSelectList=function (list) {
		...
		if (lastApprove) {
			...
			_this.$ajax({
				url:"/"+dbpath+"/getJsonOrginfo?openagent",
				dataType:"json",
				type:"post",
				async:false,
				data:{
		...
		if (cooperPosition) {
			...
			list["cooperposition"]=option.value.split("|");
		}
	}
	;
	...
	var aprvform={
		...
		removeSavedLine:function (e) {
			...
			unid&&this.$ajax({
				url:"/"+system.userprofile_path+"/getAprvLineProcess?openagent&action=delline&unid="+unid
			},function (data,status) {
		...
		savelinedo:function (e) {
			...
			url=(newdocflag?"/"+system.userprofile_path+"/fmAprvLine?openform":"/"+system.userprofile_path+"/0/"+this.savelineUNID+"?editdocument")
			;
		...
		savelinechange:function (e) {
			...
			if(!key) return;
			_this.$ajax({
				url:"/"+system.userprofile_path+"/getAprvLineProcess?openagent&action=getline&unid="+key
			},function (data,status) {
				var isdept,__data;
				if (status=="success") {
					if (data.error) {alert(data.error); return}
