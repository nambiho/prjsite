...
// */
//*
var _request_itreqself = function (){
	...
	function drawLine(_info){
		...
		$("[id*='org_']", _active).click(function (){
			...
			///openOrg(false, _selectBox, key);
			//openOrg(false, _selectBox, key, false);
			var key = $(this).attr("id").right("org_")
			...
			if (!$("#tr_"+linerecord[i].reqStepStatus, _active).is(":hidden")){
				if ( linerecord[i].reqSetStep=="hidden" ){
					...
					if ( linerecord[i].reqHiddenSet == space || linerecord[i].reqHiddenSet == _this.apprInfo.reqstatus){
						...
						var chk_cnslt = $("#Consultation", _active)[1].checked;
						var chk_appr = false;
						if(linerecord[i].reqStepStatus.indexOf("N") == -1){
							if ( optIndex == 0 ){
								if(chk_usr == "5"){
									if(linerecord[i].reqStepStatus == "O1" || linerecord[i].reqStepStatus == "O2" || linerecord[i].reqStepStatus == "O5"){
										chk_appr = true;
									}else if(linerecord[i].reqStepStatus == "R2"){
										if(chk_cnslt) chk_appr = true;
									}
								}else{
									if(linerecord[i].reqStepStatus == "O2" || linerecord[i].reqStepStatus == "O5"){	
										chk_appr = true;
									}else if(linerecord[i].reqStepStatus == "R2"){
										if(chk_cnslt) chk_appr = true;
									}
